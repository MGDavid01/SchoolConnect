...
interface CalendarEvent {
  dotColor: string;
  // Agrega aquí otras propiedades que tenga el evento si las hay
}

interface EventType {
  color: string;
  label: string;
  // Agrega aquí otras propiedades que tenga el tipo de evento si las hay
}
...
const EventNotifications: React.FC = () => {
  const upcomingEvents: UpcomingEvent[] = Object.entries<CalendarEvent>(CALENDAR_EVENTS_2024_2025)
    .filter(([date]) => {
  ...
  (type: EventType) => type.color === event.dotColor
);
      return {
        date,
