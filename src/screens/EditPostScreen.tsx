...
type EditPostRouteProp = RouteProp<BlogStackParamList, 'EditPost'>;
...
const EditPostScreen = () => {
  ...
  const [tipo, setTipo] = useState<"General" | "Ayuda" | "Pregunta" | "Aviso">(post.tipo as any);
  const [visibilidad, setVisibilidad] = useState<"todos" | "grupo">(post.visibilidad as any);
  const [isSaving, setIsSaving] = useState(false);
  const [selectedImage, setSelectedImage] = useState<string | null>(post.imageUrl || null);
  ...
  const handleGuardarCambios = async () => {
    if (!contenido.trim() && !selectedImage) {
      setSnackbarMessage("Escribe algo o agrega una imagen para guardar");
    ...
    try {
      ...
      // Subir imagen a Cloudinary solo si hay una nueva imagen seleccionada
      ...
      const response = await axios.patch(`${API_URL}/api/publicaciones/${post.id}`, publicacionActualizada);

      if (response.status === 200) {
        setSnackbarMessage("¡Publicación actualizada exitosamente!");
        setSnackbarType("success");
        setSnackbarVisible(true);

        // Actualizar el post en el estado padre
        const updatedPost = {
          ...post,
      ...
      setSnackbarMessage("No se pudo actualizar. Intenta de nuevo.");
      setSnackbarType("error");
      setSnackbarVisible(true);
    } finally {
      setIsSaving(false);
  ...
  const handleCancel = () => {
    if (contenido !== post.content || tipo !== post.tipo || visibilidad !== post.visibilidad || selectedImage !== (post.imageUrl || null)) {
      Alert.alert(
        "Descartar cambios",
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity 
          ...
          style={[
            ...
            (!contenido.trim() && !selectedImage || isSaving) && styles.publishButtonDisabled
          ]}
          disabled={(!contenido.trim() && !selectedImage) || isSaving}
        >
          <Text style={[
            styles.publishButtonText,
            (!contenido.trim() && !selectedImage || isSaving) && styles.publishButtonTextDisabled
          ]}>
      ...
      >
        ...
        >
          ...
          {/* Botón para agregar imagen - solo cuando no hay imagen */}
          {!selectedImage && (
